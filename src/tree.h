...
public:
   ...
   //A entries functions--------------------
   void geteA(std::map<std::tuple<size_t, size_t>, double>& etA);
   void deleteA(size_t rowId, size_t colId);
   void inserteA(size_t rowId, size_t colId, double value);
   void reseteA(std::map<std::tuple<size_t, size_t>, double>& etA) {this->eA = etA;}
   size_t eAsize() {return this->eA.size();}
#ifndef NoRcpp   
...
private:
   ...
   std::map<std::tuple<size_t, size_t>, double> eA; // entries of A
