...
//---------------------------------------------------------------------
// Add by Xuetao Lu for computational efficiency
//---------------------------------------------------------------------

void tree::geteA(std::map<std::tuple<size_t, size_t>, double>& etA)
{
   etA = this->eA;
}

void tree::deleteA(size_t rowId, size_t colId)
{
   this->eA.erase(std::make_tuple(rowId, colId));
}

void tree::inserteA(size_t rowId, size_t colId, double value)
{
   this->eA.insert(std::make_pair(std::make_tuple(rowId, colId), value));
}
